...
const tradeMessageReceived = async (msg, uid) => {
  ...
  if(body.action === 'delete') {
    const index = userTradeContents.trades.findIndex(t => t.created === body.created)
    if(index === -1) return 'Not found, failed to delete'
    userTradeContents.trades.splice(index, 1)
    //
  } else if(body.action === 'deleteFinished') {
    userTradeContents.trades = userTradeContents.trades.filter(t => !t.finished)
    //
  } else if(body.action === 'edit') {
    const index = userTradeContents.trades.findIndex(t => t.created === body.trade.created)
    ...
    if(oldTrade.sell !== body.trade.sell) return 'Cannot change trade buy/sell'
    ...
    if(userTradeContents.trades.length >= MAX_TRADES) return 'Too many trades, please delete some'
    ...
    userTradeContents.keys = { APIkey1: body.keys.APIkey1, APIkey2: body.keys.APIkey2 }
    await db.asyncUpdate({ trader_uid: uid }, { $set: userTradeContents }, { upsert: true })
