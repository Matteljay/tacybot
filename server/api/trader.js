...
const executeTrade = async (sell, pair, amount, keys) => {
  ...
  if (!keys?.APIkey1 || !keys.APIkey2)
    return executeTradeError("API keys are required");
  const time = Date.now();
  ...
  try {
    ...
    if (!volume)
      return executeTradeError("No processed trade volume received!");
    return { volume };
  } catch (e) {
...
const trailingStage = async (trade, currentPrice, keys) => {
  ...
  if (trade.sell) {
    if (currentPrice < trade.peekPrice * (1 - trade.percent / 100)) {
      trade.active = false;
      const useVolume =
        trade.type === "tmit" ? trade.volumeReserved : trade.volume;
      debug("trail: " + currentPrice);
      const { volume, err } = await executeTrade(
        true,
        trade.pair,
        useVolume,
        keys
...
const tradeContentPerUser = async (tradeContent) => {
  ...
  await db.asyncUpdate(
    { trader_uid: tradeContent.trader_uid },
    { $set: { trades: tradeContent.trades } }
  );
  // Message users from the connectedUsers
  //console.log(this.connectedUsers) //DEBUG
  const onlineUser = this.connectedUsers.find(
    (trader) =>
      trader.uid === tradeContent.trader_uid &&
      trader.ws.readyState === WebSocket.OPEN
  );
  if (onlineUser)
    onlineUser.ws.send(JSON.stringify({ trades: tradeContent.trades }));
};
...
const pullAccountInfo = async (keys) => {
  ...
  try {
    const response = await axios.get(
      "https://api.binance.com/sapi/v1/capital/config/getall?" + signedMsg,
      { headers: { "X-MBX-APIKEY": keys.APIkey1 } }
    );
...
const mainTimer = async () => {
  ...
  const aggTradeContents = await db.asyncFind({
    trader_uid: { $exists: true },
  });
...
const refreshSubscriptions = (aggTradeContents) => {
  ...
  // Build arrays
  const delPairs = this.currentPairs.filter(
    (pair) => !neededPairs.includes(pair)
  );
  const addPairs = neededPairs.filter(
    (pair) => !this.currentPairs.includes(pair)
  );
  // Build messages & send
  if (delPairs.length) {
    const delMessage = {
      method: "UNSUBSCRIBE",
      params: delPairs.map((p) => p.toLowerCase() + "@ticker"),
      id: 1,
    };
  ...
  if (addPairs.length) {
    const addMessage = {
      method: "SUBSCRIBE",
      params: addPairs.map((p) => p.toLowerCase() + "@ticker"),
      id: 1,
    };
...
const openSocketAndSubscribe = () => {
  ...
  this.bnbSocket = new WebSocket("wss://data-stream.binance.com/stream");
  this.bnbSocket.onerror = async (err) => {
    log("Binance socket error: " + err.message);
  };
...
if (!this.mainTimer_instance)
  this.mainTimer_instance = setInterval(mainTimer, 5000);
