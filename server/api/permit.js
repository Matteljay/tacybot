...
const hash = (email, pass) => sha512(email + pwSalt + pass).toString()
...
const roleEqualHigher = (user, testRole) => roles.indexOf(user.role) >= roles.indexOf(testRole)
...
const WSAuthMiddleware = (ws, req, next) => {
  ...
  jsonWebToken.verify(token, jwtSecret, async (err, jwt) => {
    ...
    if(!user.approved) return log('WS rejected unapproved user: ' + user.email)
    req.jwt = jwt
    req.user = user
    next()
  })
...
const authMiddleware = (req, res, next) => {
  ...
  jsonWebToken.verify(token, jwtSecret, async (err, jwt) => {
    if(err) return res.status(403).send('Browser token expired, please log in again')
    const user = await db.asyncFindOne({ _id: jwt.uid })
    if(!user) {
...
router.post('/createUser', createUserLimiter, async (req, res) => {
  ...
  const newUser = { email: body.email.trim().toLowerCase(), password: body.password }
  ...
  if(existingEmail) {
    if(existingEmail.approved) {
      ...
      return res.send('Your account already exists and is approved! Please log in.')
    }
    log('account exists, ignored: ' + newUser.email + ' uid: ' + existingEmail._id)
    return res.send('Account is waiting for approval. The server admin will contact you.')
  }
  ...
  if(newUser.approved) {
    ...
    res.send('Thanks for your registration. The server admin will contact you.')
  }
})
...
router.post('/login', async (req, res) => {
  ...
  if(badField(body.email, MIN_EMAIL_LENGTH)) return res.sendStatus(417)
  if(badField(body.password, MIN_PASSWORD_LENGTH)) return res.sendStatus(417)
  const logUser = { email: body.email.trim().toLowerCase(), password: body.password }
  ...
  const token = jsonWebToken.sign({ uid: existingUser._id }, jwtSecret, { expiresIn: jwtExpire })
...
router.post('/changeUserinfo', authMiddleware, async (req, res) => {
  ...
  if(user.password !== passHash) {
    ...
    return res.status(401).send('Incorrect old password for user: ' + user.email)
  }
  const modUser = {}
  ...
  if('email' in body) {
    ...
    if(existingEmail && existingEmail._id !== user._id) {
      ...
      return res.status(401).send('New email already exists, please choose another.')
    }
    if(!('newPassword' in body)) {
      log('changeinfo: tried to change email without password')
      return res.status(401).send('Please provide any new password when changing your e-mail')
    }
  }
  if('newPassword' in body) {
    if(badField(body.newPassword, MIN_PASSWORD_LENGTH)) return res.sendStatus(417)
...
router.get('/backupDB', authMiddleware, async (req, res) => {
  if(!roleEqualHigher(req.user, 'owner')) return res.sendStatus(403)
  const backupFileName = (new Date).toISOString().substring(0, 10) + '_' + databaseFileName
  try {
    await compactDB()
...
router.post('/resetPass', authMiddleware, async (req, res) => {
  if(!roleEqualHigher(req.user, 'owner')) return res.sendStatus(403)
  if(badField(req.body.password, MIN_PASSWORD_LENGTH)) return res.sendStatus(417)
  const target = await getTargetUser(req, res)
  if(!target) return
  newPassHash = hash(target.email, req.body.password)
  await db.asyncUpdate({ _id: target._id }, { $set: { password: newPassHash } })
...
router.post('/toggleApproved', authMiddleware, async (req, res) => {
  ...
  log('approved status set to ' + newStatus + ' for user: ' + target.email + ' by: ' + req.user.email)
...
const getTargetUser = async (req, res) => {
  ...
  if(roleEqualHigher(target, req.user.role)) {
    log('denied ' + req.url.substring(1) + ' for: ' + target.email + ' by: ' + req.user.email)
...
module.exports = { router, openDB, getDB, closeDB, authMiddleware, WSAuthMiddleware }
